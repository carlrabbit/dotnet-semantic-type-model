...
namespace SemanticTypeModel.TestModels.ModelA.Intake;
...
[SemanticType(SemanticTypeRole.Entity)] public sealed record WorkflowSpecification : Specification;

[SemanticType(SemanticTypeRole.Entity)]
public sealed record ImportSpecification : Specification
{
    public required ImportType ImportType { get; init; }
    public ImportType? OptionalImportType { get; init; }
    [SemanticOwned] public required DeliveryContract DeliveryContract { get; init; }
    [SemanticOwned] public required ScheduleContract Schedule { get; init; }
    [SemanticOwned] public required SourcePollingPolicy Polling { get; init; }
    [SemanticOwned, SemanticRequiredWhen(nameof(ImportType), nameof(ImportType.CsvFile))] public CsvSourceSpecification? CsvSource { get; init; }
    [SemanticOwned, SemanticRequiredWhen(nameof(ImportType), nameof(ImportType.XmlFile))] public XmlSourceSpecification? XmlSource { get; init; }
    [SemanticOwned, SemanticRequiredWhen(nameof(ImportType), nameof(ImportType.WebService1))] public PrimaryApiSource? WebService1Source { get; init; }
    [SemanticOwned, SemanticRequiredWhen(nameof(ImportType), nameof(ImportType.WebService2))] public SecondaryApiSource? WebService2Source { get; init; }
    [SemanticOwned] public required PostProcessingContract PostProcessing { get; init; }
...
{
    [SemanticVersion] public required int SchemaVersion { get; init; }
    [JsonExtensionData, SemanticExtensionData] public Dictionary<string, JsonElement>? ExtensionData { get; init; }
}
