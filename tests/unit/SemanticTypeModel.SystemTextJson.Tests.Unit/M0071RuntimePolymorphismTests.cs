...
{
    [Test]
    public async Task Plain_options_round_trip_entity_and_guid_scalar()
    {
        var options = new JsonSerializerOptions(JsonSerializerDefaults.Web);
        _ = options.AddSemanticTypeModelJson(ModelAGenerated.ModelASemanticTypeModel.Create());

        var value = new ModelA.SpecialEntity { Id = Guid.NewGuid(), SpecialId = Guid.Parse("11111111-1111-1111-1111-111111111111") };
        ...
        _ = await Assert.That(((ModelA.SpecialEntity)roundTripped!).SpecialId).IsEqualTo(Guid.Parse("11111111-1111-1111-1111-111111111111"));
    }
    ...
    public async Task Multiple_independent_models_compose_in_either_registration_order()
    {
        ...
        var a = JsonSerializer.Serialize<ModelA.BaseEntity>(new ModelA.SpecialEntity { Id = Guid.Empty, SpecialId = Guid.Empty }, first);
        var b = JsonSerializer.Serialize<ModelB.BaseEntity>(new ModelB.SpecialEntity { Id = "B", OtherId = Guid.Empty }, second);
