...
## M0051 application boundary (superseded by M0052)

The M0051 augmentation framing is superseded. Closed application now suppresses semantic-only extension-data CLR members from explicit `EfCoreSemanticModel` lineage and treats owned semantic value objects as reachable owned metadata; semantic value objects never become independent roots and are invalid root `DbSet<T>` types.

## M0052 closed ModelBuilder application

`DeriveEfRelationalModel` remains an inspection contract. Static CLR application uses the versioned compile-time semantic manifest and generated ordinary EF configurations defined by `ef-core-generated-configuration-contract.md`; runtime closed-`ModelBuilder` application is removed in 3.0.
