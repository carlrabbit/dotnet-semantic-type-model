# Building Blocks

## Purpose

Building blocks define incremental engineering capabilities.

## Applied Blocks

| Block | Name | Status |
|---|---|---|
| BB00 | Repository Base | Applied |
| BB01 | .NET Solution | Applied |
| BB02 | Shared Build Configuration | Applied |
| BB03 | EditorConfig and C# Style | Applied |
| BB04 | MTP + TUnit Unit Tests | Applied |
| BB05 | Test Guardrails | Applied |
| BB06 | BenchmarkDotNet | Applied |
| BB07 | GitHub Actions CI | Applied |
| BB08 | Agent Instructions | Applied |
| BB13 | Documentation Skeleton | Applied |

## Available but Not Applied

| Block | Name | Reason |
|---|---|---|
| BB09 | Bun + Biome | Not required for pure .NET repository |
| BB10 | Blazor Module | Not applicable |
| BB11 | Playwright E2E Module | Not applicable |
| BB12 | TypeScript Runtime Tools | Not applicable |
| BB14 | NuGet Packaging | Deferred to later milestone |
| BB15 | Samples | Deferred to later milestone |
| BB16 | GitHub Copilot | Applied via .github/copilot-instructions.md |
| BB18 | GitHub Pages Website | Not applicable |
