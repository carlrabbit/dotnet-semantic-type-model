...
| Programmatic model catalog | `samples/programmatic-model-catalog/` | Dynamic canonical authoring -> occurrence-diverse Random, TestData sampling profiles, coordinated TestData, and Semantic Terminology Profile-guided semantic TestData -> deterministic inspection, including `random-diversity`, `sampling-profile`, `coordinated-generation`, and unsupported diagnostics |
...
dotnet run --project samples/programmatic-model-catalog -- coordinated-generation
